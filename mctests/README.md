...
| [`pokeblocks.resourcepack.mctest.yml`](pokeblocks.resourcepack.mctest.yml) | The **served pack** is built, bundles the mod's built-in doll assets (the version-skew guarantee for doll-only updates), and carries the server's display overrides. | `resourcepack.built` on a clean server (no admin custom assets), entry-count floors under `geo/block/`, `textures/block/` and `animations/block/` inside the built zip, exactly one `server_overrides.json`, plus a bogus-prefix negative control. Also proves `AssetScanner`'s jar-URI walk works from a real mod jar. |
