...
public abstract class DataFixesInternals {
    ...
    public static <T> int getModDataVersion(@NotNull Dynamic<T> dynamic) {
        return dynamic.get(PokeblocksCommon.MOD_ID + "_DataVersion").asInt(1);
    }
    ...
    public static @NotNull DataFixesInternals get() {
        if (instance == null) {
            ...
            if (latestVanillaSchema == null) {
                PokeblocksCommon.LOGGER.warn("[{} DFU] Failed to initialize! Either someone stopped DFU from initializing", PokeblocksCommon.MOD_ID);
